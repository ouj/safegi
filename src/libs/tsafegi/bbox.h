...
struct BBox : public Shape {
    ...
    virtual isect<shape_s> intersect(const ray<shape_s>& r) {
        if(!_hitbbox(r)) return isect<shape_s>();

        isect<shape_s> is; mreal<length_d> mint;
    ...
    virtual boundingbox<shape_s> bound() { return bbox; }

private:
    bool _hitbbox(const ray<shape_s>& r) {
        mreal<length_d> tmin = r.tmin, tmax = r.tmax;
    ...
    boundingbox<shape_s> _bound(const std::vector<shared_ptr<Shape>>& s) {
        boundingbox<shape_s> bbox = s[0]->bound();
