...
// uses dememsion_signature to retrieve the signature of dimension. 
// the value of the sig string is generated when template deductiong happens.
template<typename D>
...
struct dimension_signature<dimension<da1,da2,da3,da4,da5,da6,da7,da8>>
{
    ...
    static inline const string get(){ return string(sig); }
};
// compile time genertated constant signature string
template<int da1, int da2, int da3, int da4, int da5, int da6, int da7, int da8>
...
// space_signature type to retrieve the space signature by using template specialization
template<typename S>
struct space_signature
{
    inline static const string get();
};
// a magical macro defines template specialization for each space
#define space_signature_define(NAME_S) template <> inline const string space_signature<NAME_S>::get() { return string(#NAME_S); }
...
// type_signature returns type signature by 
template <typename S>
